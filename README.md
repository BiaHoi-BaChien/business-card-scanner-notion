...
共有ホスティングなど CLI が使えない環境でも 500 エラーを避けて起動できるよう、以下を順に確認してください。

1. デプロイ先サーバーの `register_business_card` ディレクトリ直下で依存関係をインストールし、`vendor/` と `bootstrap/cache/services.php` などを生成します。
   - CLI が使える場合: `composer install --no-dev --optimize-autoloader`
   - CLI が使えない場合: ローカルで上記コマンドを実行して `vendor/` を作成し、サーバーへアップロードします。
2. `storage/` と `bootstrap/cache/` に Web サーバーからの書き込み権限があることを確認し、足りなければ適切な権限を付与します。
3. インストール後にブラウザで `/register_business_card/public/index.php` へアクセスし、500 エラーが解消されていることを確認します。
